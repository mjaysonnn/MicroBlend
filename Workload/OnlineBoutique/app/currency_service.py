...
def change_to_another_currency(total_cost, output_currency):
    ...
    # with tracer.start_as_current_span("ChangeCurrency", parent_ctx, kind=trace.SpanKind.SERVER):
    # Get the rate of the output currency
    output_rate = currency_dict[output_currency]
    # Convert the total cost to the output currency
    output_cost = float(total_cost) * float(output_rate)

    # logger.info("The total cost of the product is {} {}".format(output_cost, output_currency))
    return output_cost
...
def run_currency_service(input_p: Union[str, None] = None):
    ...
    cost = change_to_another_currency(total_cost, output_currency)
